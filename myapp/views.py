...
def about(request):
    a='This is a course registering app!!'

    return render(request,'myapp/about.html',{'a':a})
...
def user_login(request):
    if request.method == 'POST':
        ...
        if 'visits' in request.session.keys():  # Setting the counter in login page
            request.session['visits'] = request.session['visits'] + 1
        else:
            request.session['visits'] = 0
        form=LoginForm()
        return render(request, 'myapp/login.html',{'requser':request.user,'form':form,'visits': request.session['visits']})
